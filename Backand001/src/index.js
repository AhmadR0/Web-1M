...
app.post("/data", (req, res) => {
  ...
  connection.query(getLastSensorDataQuery, (err, result) => {
    ...
    if (result.length > 0) {
      ...
      // Mengambil nilai terakhir dari kolom stdrKWH pada tabel prngtandaya
      ...
      //       const message = `Nilai KWH telah mencapai batas peringatan: ${peringatanKwh}`; // Isi pesan peringatan
...
app.get("/kirim-esp", (req, res) => {
  // Query untuk mengambil data dari tabel stdrBiaya
  // const query = "SELECT stdrHarga FROM stdrBiaya LIMIT 1";
